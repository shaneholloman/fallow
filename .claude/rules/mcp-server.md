...
- `get_hot_paths` - runtime-context slice over the same `fallow health --runtime-coverage` pipeline. Same params as `check_runtime_coverage`; same free-vs-paid contract (single local capture free, multi-capture paid). Steers agents to read `runtime_coverage.hot_paths`, sorted by percentile and invocation count. Use `top` to cap returned hot paths. Always emits a top-level `warnings` array (empty when none).
