...
if (.findings | length) == 0 then
  ...
  "| File | Function | Severity | Cyclomatic | Cognitive | Lines |\n|:-----|:---------|:---------|:-----------|:----------|:------|\n" +
  ([.findings[:25][] |
    "| `\(.path):\(.line)` | `\(.name)` | \(.severity // "moderate") | \(.cyclomatic)\(if .exceeded == "cyclomatic" or .exceeded == "both" then " **!**" else "" end) | \(.cognitive)\(if .exceeded == "cognitive" or .exceeded == "both" then " **!**" else "" end) | \(.line_count) |"
  ] | join("\n")) +
